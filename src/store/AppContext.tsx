...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addGamaeya = async (gamaeya: Omit<import('../types').Gamaeya, 'id' | 'createdAt' | 'status' | 'payments'>) => {
    ...
    if (user) {
      ...
      setState((prev) => ({ ...prev, gamaeyas: [...(prev.gamaeyas || []), newGamaeya] }));
    }
  };
  ...
  const payGamaeyaMonth = async (gamaeyaId: string, monthIndex: number) => {
    ...
    const gamaeya = list.find(g => g.id === gamaeyaId);
    if (!gamaeya) return;
    ...
    try {
      ...
      if (user) {
        ...
        if (newExpense.accountId) {
          const accRef = doc(db, 'users', user.uid, 'accounts', newExpense.accountId);
          const currentAcc = state.accounts.find(a => a.id === newExpense.accountId);
          if (currentAcc) {
    ...
    const updatedPayments = (gamaeya.payments || []).map(p => {
    ...
    const isAllPaid = updatedPayments.every(p => p.paid);
    ...
    try {
      ...
      if (user) {
        ...
        if (newIncome.accountId) {
          ...
          const currentAcc = state.accounts.find(a => a.id === newIncome.accountId);
          if (currentAcc) {
